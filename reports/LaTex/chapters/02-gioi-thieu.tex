...
\begin{itemize}
    ...
    \item \textbf{SNMP, NetBIOS, MSSQL, Portmap:} hệ số từ 4x đến $\sim$30x.
\end{itemize}

Nhóm tấn công này có đặc điểm lưu lượng chiều về nạn nhân rất lớn, gói tin UDP, port đặc thù của từng giao thức, và nguồn là các máy chủ hợp pháp trên Internet. Trong bộ dữ liệu CIC-DDoS 2019, các loại tấn công này tạo thành nhóm \textbf{G1} (Reflection/Application: DNS, LDAP, MSSQL, NetBIOS, SNMP, Portmap).

\subsubsection{Tấn công khuếch đại thể tích -- NTP/TFTP (Reflection/Volume)}

Bên cạnh các tấn công khuếch đại giao thức ứng dụng, một số giao thức tạo ra hệ số khuếch đại thể tích đặc biệt cao:
\begin{itemize}
    \item \textbf{NTP Amplification:} Khai thác lệnh \texttt{monlist} của giao thức NTP để khuếch đại lưu lượng lên tới $\sim$556x -- hệ số cao nhất trong các kỹ thuật phản chiếu phổ biến.
    \item \textbf{TFTP Amplification:} Giao thức truyền file đơn giản (UDP/69); kẻ tấn công giả mạo IP nạn nhân gửi yêu cầu đọc file, máy chủ TFTP phản hồi nhiều gói dữ liệu liên tiếp về phía nạn nhân.
\end{itemize}

Trong bộ dữ liệu CIC-DDoS 2019, NTP và TFTP tạo thành nhóm \textbf{G2} (Reflection/Volume) với khối lượng lưu lượng lớn nhất trong ba nhóm thí nghiệm.
...
Trong bộ dữ liệu CIC-DDoS 2019, nhóm này bổ sung thêm \textbf{UDP Flood} và \textbf{UDP-Lag} -- tấn công làm cạn kiệt tài nguyên xử lý gói tin của hệ thống đích -- tạo thành nhóm \textbf{G3} (Exploitation: Syn, UDP, UDPLag, WebDDoS).
