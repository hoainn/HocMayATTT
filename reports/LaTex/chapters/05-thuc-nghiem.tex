...
\begin{table*}[!t]
\caption{Siêu tham số tốt nhất do Optuna TPE tìm được (30 trial, subsample 50.000 mẫu/trial).}
...
\hline
\multirow{3}{*}{Exp 1 (G1)} & KNN & \texttt{n\_neighbors=3, weights=distance, metric=manhattan} \\
...
\hline
\multirow{3}{*}{Exp 2 (G2)} & KNN & \texttt{n\_neighbors=5, weights=distance, metric=manhattan} \\
...
\hline
\multirow{3}{*}{Exp 3 (G3)} & KNN & \texttt{n\_neighbors=3, weights=distance, metric=manhattan} \\
...
Lưu ý: thời gian suy luận của KNN dao động không đơn điệu theo kích thước tập huấn luyện (Exp 3 cao hơn Exp 1 dù train size lớn hơn). Nguyên nhân là sklearn tự động lựa chọn thuật toán tìm kiếm hàng xóm (brute-force, kd-tree, hoặc ball-tree) dựa trên kích thước và mật độ tập huấn luyện, kết hợp với giá trị tham số $k$ được Optuna chọn khác nhau cho từng thí nghiệm. Do đó, thời gian suy luận của KNN không thể so sánh trực tiếp giữa các thí nghiệm.
